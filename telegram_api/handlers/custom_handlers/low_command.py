...
from telegram_api.states.custom_commands_states.low_command_states import SearchStates
from site_api.utils.output_data_generator import nearest_places
...
def cmd_start(message: Message) -> None:
    """
    Хэндлер.
    Срабатывает, если пользователь отправил команду /low.
    ...
    if not query.exists():
        ...
        bot.set_state(message.from_user.id, SearchStates.low_cmd_category, message.chat.id)


@bot.message_handler(state=SearchStates.low_cmd_category)
def get_category(message: Message) -> None:
    ...
    if not message.text.isdigit():
        ...
        bot.set_state(message.from_user.id, SearchStates.low_cmd_results_quantity, message.chat.id)
...
@bot.message_handler(state=SearchStates.low_cmd_results_quantity)
def get_results_quantity(message: Message) -> None:
    ...
    if message.text.isdigit():
        ...
        if not 1 <= places_to_show_quantity <= 10:
            ...
            results = nearest_places(limit=places_to_show_quantity).search_for_nearest_places()
            ...
            data = f"('{current_datetime}', {message.from_user.id}, '/low', '{place}', '{search_result}')"
